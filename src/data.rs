...
impl GridVariable{
    fn new_from_parts(data: Array<f64, IxDyn>, name: &str, dims: &[&str], attr: Option<HashMap<String,String>>) -> Self{
...
impl DataSet{
    ...
    pub fn to_netcdf(&self, f: PathBuf) -> Result<()>{
        ...
        for v in self.vars.iter(){
            ...
            let var = &mut file.add_variable::<f64>(&v.name, 
                &dim_names)?;
            
        }
        for v in self.vars.iter(){
            let mut var = file.variable_mut(&v.name).ok_or(anyhow!("Can't find variable"))?;
            let data = v.data.as_slice().ok_or(anyhow!("Can't convert array to slice"))?;
            var.put_values(data, ..)?;
        }
...
mod tests{
    ...
    fn create_var() -> GridVariable{
        ...
        let var = GridVariable::new_from_parts(
            data,
            "test",
            &["x", "y"],
            None );
